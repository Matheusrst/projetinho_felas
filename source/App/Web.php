...
class Web extends Controller
{
    ...
    public function blogCategory(array $data): void
    {
        ...
        $head = $this->seo->render(
            ...
            ($category->cover ? image($category->cover, 1200, 628) : theme("/assests/image/shere.jpg"))
        );
    ...
    public function login(?array $data): void
    {
        ...
        if (!empty($data['csrf'])) {
            ...
            if (request_limit("weblogin", 3, "60 * 5")) {
                $json['message'] = $this->message->error("3 tentativas efetuadas com falha, volte em 5 minutos e tente novamente")->render();
                echo json_encode($json);
                return;
            }
    ...
    public function forget(?array $data)
    {
        ...
        if (!empty($data['csrf'])) {
            ...
            if (request_repeat("webforget", $data["email"])) {
                $json['message'] = $this->message->error("você ja tentou este email antes")->render();
                echo json_encode($json);
                return;
            }
